...
#[derive(Parser)]
#[command(name = "gym", version, about = "Hanzo Gym — LoRA fine-tuning and GRPO on hanzo-ml")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::from_default_env().add_directive("gym=info".parse()?))
        .init();
    match Cli::parse().cmd {
        Cmd::Train { config } => gym::train::run(&gym::Config::load(config)?),
        Cmd::Preprocess { config } => gym::data::preprocess(&gym::Config::load(config)?),
        Cmd::Merge { config, out } => gym::model::merge(&gym::Config::load(config)?, out.as_deref()),
    }
}
